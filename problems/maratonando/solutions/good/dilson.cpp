...
struct Edge{
	...
	int cap;
    bool real;
	Edge(int v_, int cap_, int rev_, bool real_) : v(v_), rev(rev_), cap(cap_), real(real_) {}
};
...
int BuildTreeAndCountMatches(const vector<vector<int>>& g, const vector<int>& matches, const int node, vector<int>& parent, vector<int>& color) {
    color[node] = color[parent[node]];
    int count = 0;
    if (matches[node] != -1 && parent[matches[node]] == -1) {
        parent[matches[node]] = node;
        count += BuildTreeAndCountMatches(g, matches, matches[node], parent, color) + 1;
    }
...
int main() {
    ...
    for (int r = 0; r < n + e; r++) {
        if (parent[r] == -1) {
            parent[r] = r;
            color[r] = r;
            const int num_matches = BuildTreeAndCountMatches(g, matches, r, parent, color);
