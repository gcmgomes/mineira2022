...
void Connect(vector<vector<int>>& graph, vector<int>& matching,
             vector<int>& left) {
  int root = 0;
  for (auto& u : left) {
    if (matching[u] != -1) {
  ...
  vector<int> component(graph.size(), 0), ignored(graph.size(), 0);
  component[root] = 1;
  queue<int> candidates;
  while (CBFS(graph, matching, component, ignored, candidates, root)) {
    while (candidates.size()) {
      int v = candidates.front();
      candidates.pop();
      bool swapped = 0;
      for (auto& w : graph[v]) {
        if (component[matching[w]] == 0) {
          matching[matching[w]] = -1;
          matching[w] = v;
          matching[v] = w;
          root = v;
          component[root] = 1;
