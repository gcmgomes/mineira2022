...
def MakeSeparator(prev_last):
    graph = [[prev_last + 1],
             [prev_last + 2],
             [prev_last, prev_last + 1],
             [prev_last + 1, prev_last + 2],
             [prev_last + 2, prev_last + 3]]
    return graph
...
s=int(sys.argv[4])
# fifth argument is the probability of each extra edge.
p=float(sys.argv[5])
...
for i in range(0, s+1):
    if i > 0:
        graphs.append((5, MakeSeparator(offset)))
        offset += 2
    graphs.append((n, RebaseGraph(MakeGraph(n, e, p), offset - n + 1)))
    offset += e
print(total_n,(s+1)*e + s*2)
for ng in graphs:
    n_g, graph = ng
